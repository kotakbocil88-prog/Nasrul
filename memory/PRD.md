...
- DONE (2025-07 v15): TEMA OTOMATIS. index.js kini menerapkan tema sebelum paint berdasarkan: pilihan manual tersimpan (localStorage 'theme') jika ada, jika belum -> ikuti tema perangkat via matchMedia('(prefers-color-scheme: dark)'). Dashboard menambah listener matchMedia 'change' yang mengikuti perubahan tema perangkat secara real-time SELAMA user belum memilih manual (menghormati pilihan manual). Sekalian polish dark-mode: teks near-black text-[#0B1D15] diberi dark:text-emerald-50 (replace_all, semua di kartu); text-emerald-700 heading kartu -> dark:text-emerald-400; sub-kartu Status Tagging (bg-white/emerald-50/amber-50 + active ring) dibuat dark-aware; chip ikon bg-emerald-50 -> dark:bg-emerald-500/10. QR container tetap bg-white (wajib agar QR ter-scan). Verified: emulate color_scheme=dark -> auto dark True (saved None), semua angka statistik & kartu terbaca; lint clean.
- DONE (2025-07 v7): Batas ambil data dinaikkan dari 5000 -> 20000 di semua query records (GET /records, /records/stats, dan _fetch_docs untuk export PDF/Excel). Frontend tetap pakai pagination atas seluruh data yang diambil (tanpa cap). Backend restart OK, login admin@eqms.id verified.
- DONE (2025-07 v8): Koordinat dibatasi maksimal 6 angka di belakang koma. Backend: fmt_num() dipangkas ke 6 desimal (buang nol berlebih) + helper round_coords() dipanggil di create/update/import_confirm sehingga nilai tersimpan sudah dibulatkan 6 desimal. Id Actual & isi QR (build_id_actual) otomatis ikut 6 desimal. Frontend RecordDialog.fmtNum() juga dipangkas 6 desimal agar preview Id Actual live cocok. Verified: create 110.40011399/0.65452167 -> 110,400114/0,654522.
- DONE (2025-07 v9): (a) EKSPOR TERPILIH — dropdown Ekspor punya toggle scope "Semua (N)" / "Hasil filter (N)"; PDF Tabel & Ekspor Excel mengirim filteredIds bila scope=filter (endpoint POST /export/{table,excel} sudah dukung ids). Pratinjau & Cetak Label memang sudah pakai filtered. (b) IMPOR LEBIH CEPAT — backend import_confirm pakai insert_many (bulk, jauh lebih cepat); frontend ImportDialog mengirim per-batch 1000 baris dengan progress bar determinate + label persen. Verified backend: insert_many 2 baris OK, rounding impor 100.1234567->100.123457, filtered excel export ids http200. Frontend compiled clean.
- DONE (2025-07 v10): (a) RINGKASAN BLOK TERTAGGING di dashboard (section blok-tagging-summary-section, judul "Status Tagging per Blok"): identitas blok = Kebun+Afdeling+Blok; "sudah tertagging" = semua titik sample punya koordinat, "belum" = ada ≥1 titik kosong (subteks: N sebagian · M kosong) + progress bar + persen. Reaktif ikut filter (baseFiltered). (b) RIWAYAT IMPOR: preview kirim skipped (parse_excel return (parsed, skipped)); ImportDialog tampil "Terbaca N · X dilewati", progress bar per-batch 1000, dan daftar Riwayat Impor sesi (+N masuk, X dilewati, jam). (c) EKSPOR TERPILIH: toggle scope Semua/Hasil filter di dropdown Ekspor -> PDF Tabel & Excel kirim filteredIds. (d) CETAK TERFILTER AMAN: banner print-warning bila >200 label + window.confirm sebelum window.print. Semua diverifikasi frontend testing agent 4/4 PASS (12.525 record produksi, read-only).

- DONE (2025-07 v10): Formula Id Actual diperbaiki — Code LSU DIGANTI dengan Titik Sample. Formula baru = Kebun + Afdeling + Blok + Titik Sample + Koord_X + Koord_Y (tanpa pemisah, koordinat koma desimal). QR content = Id Actual (ikut berubah). Diterapkan di backend build_id_actual() + frontend (Dashboard qrContent & RecordDialog live preview). Backend tested 5/5 pass (id_actual pakai titik_sample & abaikan code_lsu, PUT recompute, ekspor regresi 200, data produksi 4699 aman).

- DONE (2025-07 v11): QR Ganda Cek — deteksi & tandai duplikat berdasarkan Id Actual/isi QR (payloadOf). Badge merah "Ganda" (badge-dup-{id}) di kolom Id Actual + toggle cepat "Ganda" (filter-dup-toggle) dgn hitungan (dup-count-badge) untuk menyaring hanya baris duplikat; terintegrasi ke chip filter, reset, page reset. Verifikasi UI 100%: preview Id Actual pakai Titik Sample (bukan Code LSU) benar di form & tabel; deteksi duplikat bekerja (produksi terdeteksi 2062 baris duplikat). Cleanup record uji ZZUI OK.

- DONE (2025-07 v12): Ringkasan Agronomi — format desimal rata-rata diseragamkan jadi 1 desimal untuk LAI, Panjang Pelepah, SPH, Jumlah Pelepah (sebelumnya nol di belakang koma terbuang sehingga SPH/Panjang Pelepah tampak tanpa desimal). fmtStat pakai toFixed(digits) agar trailing zero dipertahankan; AGRO_METRICS digits semua=1. Berlaku ke avg/min/maks + rincian per Kebun/Afdeling. Diverifikasi via screenshot.

- DONE (2025-07 v13): Fitur "QR Ganda Cek" (v11) DIHAPUS atas permintaan user — toggle 'Ganda' (filter-dup-toggle), badge merah 'Ganda' (badge-dup), state onlyDup, dupKeys/dupCount, serta referensi di chip/criteria/reset/page-reset & import Copy dibersihkan. Verifikasi via screenshot: toggle tidak ada lagi, toggle 'Baru' & fitur lain tetap normal. Lint clean.
